/**
 * Benchmark server using ClusterManager.
 *
 * Master process spawns N workers via ClusterManager.
 * Each worker binds to the same port with reusePort.
 *
 * stdout signal: WRK_READY:<port>  (emitted by master once workers are up)
 */
import {
  ClusterManager,
  Application,
...
const WORKER_COUNT = process.env.WORKERS ? Number(process.env.WORKERS) : ('auto' as const);
...
if (!ClusterManager.isWorker()) {
  ...
  const wid = ClusterManager.getWorkerId();
  const app = new Application({ port: PORT, reusePort: true });

  app.registerExtension(
    new LoggerExtension({ prefix: `W${wid}`, level: LogLevel.ERROR }),
  );
  app.use(createLoggerMiddleware({ prefix: `[W${wid}]`, logger: () => {} }));
