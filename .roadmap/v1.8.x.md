...
- [ ] 实现对象验证规则（`IsObject`, `ValidateNested`, `IsNotEmpty`,
      `IsNotEmptyObject`）
- [ ] 实现数组验证规则（`IsArray`, `ArrayMinSize`, `ArrayMaxSize`,
      `ArrayUnique`, `ArrayContains`）
- [ ] 实现条件验证规则（`ValidateIf`）
- [ ] 实现类型转换规则（`Transform`）
- [ ] 实现其他常用规则（`IsUUID`, `IsDate`, `IsBoolean`, `IsInt`, `Min`, `Max`,
      `Length`, `Matches`, `IsIn`, `IsNotIn`）
- [ ] 实现自定义验证器工厂（`createCustomValidator`）
- [ ] 实现 DTO 类级别验证（`@ValidateClass()`）
- [ ] 嵌套对象验证支持
- [ ] 添加单元测试（`tests/validation/`）
- [ ] 更新文档
...
- 新增 `src/validation/custom-validator.ts`
- `tests/validation/` 扩展测试
- 更新 `src/index.ts` 导出
...
- [ ] 实现 `@Global()` 装饰器（`src/di/decorators.ts`）
- [ ] 修改 `ModuleRegistry` 支持全局模块
  - [ ] 全局模块的 exports 自动注册到根容器
  - [ ] 所有模块可直接注入全局提供者
- [ ] 修改 `Module` 元数据结构支持 `isGlobal` 标记
- [ ] 添加单元测试（`tests/di/global-module.test.ts`）
- [ ] 更新文档
...
- [ ] 对象验证（`IsObject`, `ValidateNested`）正常工作
- [ ] 数组验证（`IsArray`, `ArrayMinSize`）正常工作
- [ ] 自定义验证器工厂可用
- [ ] 嵌套对象验证支持
- [ ] 测试覆盖率 > 90%
- [ ] 文档更新
  - [ ] `docs/validation.md` 验证使用指南（新增或更新）
  - [ ] `docs/zh/validation.md` 中文文档同步
  - [ ] 更新 `docs/guide.md` 添加高级验证章节
- [ ] 示例代码
  - [ ] `examples/02-official-modules/validation-app.ts`
        验证示例（包含对象、数组、自定义验证器）

### Phase 4 验收

- [ ] `@Global()` 装饰器可用
- [ ] 全局模块的 exports 可在任何模块中使用
- [ ] 不影响现有非全局模块行为
- [ ] 测试覆盖率 > 90%
- [ ] 文档更新
  - [ ] 更新 `docs/guide.md` 添加全局模块章节
  - [ ] 更新 `docs/zh/guide.md` 中文文档同步
- [ ] 示例代码
