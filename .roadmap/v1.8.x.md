...
- [ ] **AuthGuard**：基于 `@Auth()` 元数据的认证守卫
- [ ] **RolesGuard**：基于角色的授权守卫
- [ ] **ThrottlerGuard**：请求限流守卫（与 RateLimit 中间件整合）

#### 2.4 实现任务

- [ ] 定义 `CanActivate` 接口（`src/security/guards/types.ts`）
- [ ] 定义 `ExecutionContext` 接口
- [ ] 实现 `@UseGuards()` 装饰器（`src/security/guards/decorators.ts`）
- [ ] 实现 `GuardRegistry`（守卫注册和管理）
- [ ] 集成到请求处理流程（在中间件之后、拦截器之前执行）
- [ ] 实现内置 `AuthGuard`
- [ ] 实现内置 `RolesGuard`
- [ ] 添加单元测试（`tests/security/guards/`）
- [ ] 添加集成测试
- [ ] 更新文档
...
- [ ] `@UseGuards()` 装饰器可用于控制器和方法级别
- [ ] Guards 在中间件之后、拦截器之前执行
- [ ] `AuthGuard` 和 `RolesGuard` 内置守卫可用
- [ ] 测试覆盖率 > 90%
- [ ] 文档更新
  - [ ] `docs/guards.md` 守卫使用指南
  - [ ] `docs/zh/guards.md` 中文文档同步
  - [ ] 更新 `docs/guide.md` 添加守卫章节
- [ ] 示例代码
