...
- [ ] 定义事件系统类型（`src/events/types.ts`）
- [ ] 实现 `EventEmitterService`（`src/events/service.ts`）
- [ ] 实现 `@OnEvent()` 装饰器（`src/events/decorators.ts`）
- [ ] 实现 `EventModule`（`src/events/event-module.ts`）
- [ ] 实现事件监听器自动扫描和注册
- [ ] 支持异步事件处理
- [ ] 支持事件优先级
- [ ] 支持通配符事件（可选）
...
- [ ] 文档更新
  ...
  - [ ] `docs/zh/events.md` 中文文档同步
  - [ ] 更新 `docs/guide.md` 添加事件系统章节
- [ ] 示例代码
