...
v1.8.x 版本专注于框架核心能力增强，提升框架的企业级应用能力。主要目标包括：

1. **架构文档完善**：梳理最新架构图和请求生命周期
2. **Guards 守卫系统**：实现 `@UseGuards()` 装饰器，支持认证授权守卫
3. **验证系统增强**：支持对象、数组验证和自定义验证器
4. **全局模块支持**：实现 `@Global()` 装饰器
5. **事件系统**：实现 EventEmitter 模块
...
### Phase 1: 架构文档与生命周期完善 (v1.8.0-alpha.1)

**目标**：梳理当前框架的最新架构图和请求生命周期，更新到 README
...
- [x] **更新 README.md 架构部分**
  ...
  - [x] 添加模块系统架构图
  - [x] 添加 DI 容器架构说明
  - [x] 添加执行顺序说明（中间件 → 守卫 → 拦截器 → 管道 → 处理器）

- [x] **同步更新中文文档**
  - [x] 更新 `docs/zh/guide.md`
  - [x] 确保中英文文档一致

#### 1.2 生命周期文档

- [x] **创建请求生命周期文档**（`docs/request-lifecycle.md`）
  - [x] 详细说明每个阶段的执行顺序
  - [x] 说明各组件（中间件、拦截器、过滤器）的区别
  - [x] 提供最佳实践建议

**交付物**：

- [x] 更新后的 README.md
- [x] 新增 `docs/request-lifecycle.md`
- [x] 新增 `docs/zh/request-lifecycle.md`
- [x] 更新 `.cursor/rules/architecture-overview.mdc`

---

### Phase 2: Guards 守卫系统 (v1.8.0-alpha.2)

**目标**：在 Security 模块中新增 `@UseGuards()` 装饰器，实现守卫机制
...
- [x] **AuthGuard**：基于 `@Auth()` 元数据的认证守卫
- [x] **RolesGuard**：基于角色的授权守卫
- [ ] **ThrottlerGuard**：请求限流守卫（与 RateLimit 中间件整合）-
      可选，延后实现

#### 2.4 实现任务

- [x] 定义 `CanActivate` 接口（`src/security/guards/types.ts`）
- [x] 定义 `ExecutionContext` 接口
- [x] 实现 `@UseGuards()` 装饰器（`src/security/guards/decorators.ts`）
- [x] 实现 `GuardRegistry`（守卫注册和管理）
- [x] 集成到请求处理流程（在中间件之后、拦截器之前执行）
- [x] 实现内置 `AuthGuard`
- [x] 实现内置 `RolesGuard`
- [x] 添加单元测试（`tests/security/guards/`）
- [x] 添加集成测试
- [x] 更新文档
...
- `src/security/guards/` 目录
- `@UseGuards()` 装饰器
- 内置 `AuthGuard`、`RolesGuard`
- `tests/security/guards/` 测试
- 更新 `src/index.ts` 导出

---

### Phase 3: 验证系统增强 (v1.8.0-alpha.3) ✅

**目标**：补全对象、数组验证规则，支持自定义验证器
...
- [x] 实现对象验证规则（`IsObject`, `ValidateNested`, `IsNotEmpty`,
      `IsNotEmptyObject`）
- [x] 实现数组验证规则（`IsArray`, `ArrayMinSize`, `ArrayMaxSize`,
      `ArrayUnique`, `ArrayContains`, `ArrayNotContains`, `ArrayNotEmpty`）
- [x] 实现条件验证规则（`ValidateIf`）
- [x] 实现类型转换规则（`Transform`）
- [x] 实现其他常用规则（`IsUUID`, `IsDate`, `IsBoolean`, `IsInt`, `Min`, `Max`,
      `Length`, `MaxLength`, `Matches`, `IsIn`, `IsNotIn`, `IsPositive`,
      `IsNegative`, `IsUrl`, `IsJSON`, `Equals`, `NotEquals`, `IsDefined`,
      `IsAlphanumeric`, `IsAlpha`, `IsNumberString`）
- [x] 实现自定义验证器工厂（`createCustomValidator`, `createSimpleValidator`,
      `createRegexValidator`）
- [x] 实现 DTO 类级别验证（`@ValidateClass()`, `@Property()`,
      `@NestedProperty()`, `@ArrayNestedProperty()`）
- [x] 嵌套对象验证支持
- [x] 添加单元测试（`tests/validation/`）- 129 tests, 100% pass
- [x] 更新文档

**交付物**：

- 更新 `src/validation/decorators.ts`
- 新增 `src/validation/rules/` 目录（按类别组织）
  - `object.ts` - 对象验证规则
  - `array.ts` - 数组验证规则
  - `common.ts` - 通用验证规则
  - `conditional.ts` - 条件和转换规则
- 新增 `src/validation/custom-validator.ts` - 自定义验证器工厂和内置扩展验证器
- 新增 `src/validation/class-validator.ts` - 类级别验证支持
- `tests/validation/` 扩展测试（129 个测试）
- 更新 `src/index.ts` 导出

---

### Phase 4: @Global() 模块装饰器 (v1.8.0-alpha.4) ✅

**目标**：实现 `@Global()` 装饰器，使模块的提供者可全局访问

#### 4.1 设计说明

全局模块的提供者无需在其他模块中导入即可使用。适用于：
...
- [x] 实现 `@Global()` 装饰器（`src/di/decorators.ts`）
- [x] 修改 `ModuleRegistry` 支持全局模块
  - [x] 全局模块的 exports 自动注册到根容器
  - [x] 所有模块可直接注入全局提供者
- [x] 修改 `Module` 元数据结构支持 `isGlobal` 标记
- [x] 添加单元测试（`tests/di/global-module.test.ts`）- 11 tests, 100% pass
- [x] 更新文档
...
@Module({
  ...
  // 不需要: imports: [GlobalConfigModule]
})
class UserModule {}
...
- [x] `@Global()` 装饰器实现（`src/di/decorators.ts`）
- [x] `isGlobalModule()` 辅助函数
- [x] `GLOBAL_MODULE_METADATA_KEY` 元数据键
- [x] 更新 `ModuleRegistry`（支持全局模块注册和导出）
- [x] `tests/di/global-module.test.ts`（11 个测试用例）
- [x] `examples/01-basic/global-module-app.ts` 示例代码
- [x] 更新 `docs/guide.md`（添加全局模块章节）
- [x] 更新 `docs/zh/guide.md`（中文文档同步）
- [x] 更新 `src/index.ts` 导出

---

### Phase 5: EventEmitter 事件系统 (v1.8.0)
...
- [x] 定义事件系统类型（`src/events/types.ts`）
- [x] 实现 `EventEmitterService`（`src/events/service.ts`）
- [x] 实现 `@OnEvent()` 装饰器（`src/events/decorators.ts`）
- [x] 实现 `EventModule`（`src/events/event-module.ts`）
- [x] 实现事件监听器自动扫描和注册
- [x] 支持异步事件处理
- [x] 支持事件优先级
- [x] 支持通配符事件
- [x] 添加单元测试（`tests/events/`）- 54 tests, 100% pass
- [x] 添加集成测试
- [x] 更新文档
...
- [x] README.md 包含完整的架构图和生命周期说明
- [x] 中英文文档同步更新
- [x] `.cursor/rules/architecture-overview.mdc` 更新

### Phase 2 验收

- [x] `@UseGuards()` 装饰器可用于控制器和方法级别
- [x] Guards 在中间件之后、拦截器之前执行
- [x] `AuthGuard` 和 `RolesGuard` 内置守卫可用
- [x] 测试覆盖率 > 90%（98 tests, 100% pass）
- [x] 文档更新
  - [x] `docs/guards.md` 守卫使用指南
  - [x] `docs/zh/guards.md` 中文文档同步
  - [x] 更新 `docs/guide.md` 添加守卫章节
- [x] 示例代码
  - [x] `examples/02-official-modules/guards-app.ts` 守卫示例

### Phase 3 验收

- [x] 对象验证（`IsObject`, `ValidateNested`）正常工作
- [x] 数组验证（`IsArray`, `ArrayMinSize`）正常工作
- [x] 自定义验证器工厂可用
- [x] 嵌套对象验证支持
- [x] 测试覆盖率 > 90%（129 tests, 100% pass）
- [x] 文档更新
  - [x] `docs/validation.md` 验证使用指南
  - [x] `docs/zh/validation.md` 中文文档同步
  - [ ] 更新 `docs/guide.md` 添加高级验证章节（可选，已有独立文档）
- [x] 示例代码
...
- [x] `@Global()` 装饰器可用
- [x] 全局模块的 exports 可在任何模块中使用
- [x] 不影响现有非全局模块行为
- [x] 测试覆盖率 > 90%（11 tests, 100% pass）
- [x] 文档更新
  - [x] 更新 `docs/guide.md` 添加全局模块章节
  - [x] 更新 `docs/zh/guide.md` 中文文档同步
- [x] 示例代码
  - [x] `examples/01-basic/global-module-app.ts` 全局模块示例

### Phase 5 验收

- [x] `EventModule` 可正常注册和使用
- [x] `@OnEvent()` 装饰器自动注册监听器
- [x] 同步/异步事件发布正常工作
- [x] 事件优先级正确执行
- [x] 测试覆盖率 > 90%（54 tests, 100% pass）
- [x] 文档更新
  - [x] `docs/events.md` 事件系统使用指南
  - [x] `docs/zh/events.md` 中文文档同步
  - [x] 更新 `docs/guide.md` 添加事件系统章节
- [x] 示例代码
  - [x] `examples/02-official-modules/events-app.ts` 事件系统示例

---

## 🔗 相关资源

- [v1.4.x Roadmap](./v1.4.x.md) - 微服务架构支持
- [v1.x.x 汇总清单](./v1.x.x.md) - 所有待实现功能特性汇总
- [架构概览](./.cursor/rules/architecture-overview.mdc) - 项目架构规范
...
| 日期       | 版本         | 更新内容                  |
| ---------- | ------------ | ------------------------- |
| 2026-01-21 | v1.8.0-draft | 初始规划，定义 5 个 Phase |
