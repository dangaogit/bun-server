...
- 🔧 修复 ServiceClient 先读 body 再检查 response.ok 导致的错误响应解析问题
- 🔧 修复 ConfigModule setValueByPath 方法中 null 值处理问题（typeof null ===
  'object'）
- 🔧 修复 ConfigModule 静态方法回调中 this.setValueByPath 调用导致的运行时错误
- 🔧 修复 ConfigCenterValue / ServiceDiscovery 装饰器 async getter
  导致属性读取返回 Promise 的问题
- 🔧 修复 ConfigCenter 初始化时机竞争（未注册 ConfigModule 时不应影响应用启动）
- 🔧 修复测试端口分配不稳定（getTestPort 使用 port=0 获取系统分配端口）
...
- feat(service-registry): add ServiceRegistry abstraction and Nacos
  implementation
- feat(service-client): add ServiceClient with load balancing, retry, circuit
...
- feat(decorators): add @ConfigCenterValue, @ServiceRegistry, @ServiceClient,
...
- fix(service-client): handle error responses before parsing body
- fix(config): fix setValueByPath null handling (typeof null === 'object')
- fix(config): fix static this.setValueByPath calls in callbacks
- fix(config-center): make decorator property getters synchronous
- fix(service-registry): make ServiceDiscovery decorator property getter
  synchronous
- fix(application): make config center init optional when ConfigModule is not
  registered
- fix(test): stabilize getTestPort by using port=0 probe
